...
export default function Login() {
  ...
  async function handleLogin() {
    ...
    setLoading(true)
    try {
      const { error: err, profile } = await signIn(cedula, password)
      if (err) { setError(err.message); setLoading(false); return }
      if (profile.status === 'pending' || !profile.is_active) { navigate('/pendiente'); return }
      if (!profile.security_questions_set && profile.role === 'agent') { navigate('/setup-preguntas'); return }
      setOk('Acceso correcto. Ingresando...')
      setTimeout(() => navigate(profile.role === 'admin' ? '/admin' : '/agente'), 800)
    } catch {
