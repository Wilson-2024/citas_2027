...
export default function RecuperarPassword() {
  ...
  async function step1() {
    ...
    const { data, error: err } = await supabase
      .from('profiles')
      .select('id, full_name, email_auth, security_questions_set, security_q1, security_a1, security_q2, security_a2, security_q3, security_a3, security_q4, security_a4')
      .eq('cedula', cedula)
      .single()
    setLoading(false)
    if (err || !data) { setError('No se encontró ninguna cuenta con esa cédula.'); return }
    if (!data.security_questions_set) { setError('Esta cuenta no tiene preguntas de seguridad configuradas. Contacta al administrador.'); return }
    setProfile(data)
    setStep(2)
  }
  ...
  async function step3() {
    ...
    const { error: authErr } = await supabase.auth.signInWithPassword({
      email: profile.email_auth, password: 'TEMP_BYPASS'
    })
    // Usamos admin para actualizar la contraseña
    const { error: updateErr } = await supabase.rpc('reset_agent_password', {
      agent_cedula: cedula,
      new_password: newPass
    })
    setLoading(false)
    if (updateErr) { setError('Error al actualizar. Contacta al administrador.'); return }
      ...
      <div className="wrap">
        <div className="card">
          ...
          {step === 2 && <>
            ...
            <p style={{ fontSize: 13, color: '#888', textAlign: 'center', marginBottom: '1rem', lineHeight: 1.6 }}>
              Selecciona una pregunta y respóndela correctamente
            </p>
